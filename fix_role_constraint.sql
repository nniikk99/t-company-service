-- Проверяем и исправляем ограничения ролей в user_profiles
-- Ошибка: companyResponsible не разрешена в check constraint

-- 1. Проверяем текущие ограничения ролей
SELECT 
    tc.constraint_name, 
    cc.check_clause
FROM information_schema.table_constraints tc
JOIN information_schema.check_constraints cc 
    ON tc.constraint_name = cc.constraint_name
WHERE tc.table_name = 'user_profiles' 
    AND tc.constraint_type = 'CHECK'
    AND cc.check_clause LIKE '%role%';

-- 2. Проверяем существующие роли в базе
SELECT DISTINCT role 
FROM user_profiles 
ORDER BY role;

-- 3. Удаляем старое ограничение ролей
ALTER TABLE user_profiles DROP CONSTRAINT IF EXISTS user_profiles_role_check;

-- 4. Создаем новое ограничение с правильными ролями
ALTER TABLE user_profiles ADD CONSTRAINT user_profiles_role_check 
CHECK (role IN ('superAdmin', 'administrator', 'companyResponsible', 'siteManager', 'operatorPM', 'pendingApproval'));

-- 5. Проверяем, что ограничение создано
SELECT 
    tc.constraint_name, 
    cc.check_clause
FROM information_schema.table_constraints tc
JOIN information_schema.check_constraints cc 
    ON tc.constraint_name = cc.constraint_name
WHERE tc.table_name = 'user_profiles' 
    AND tc.constraint_type = 'CHECK'
    AND cc.check_clause LIKE '%role%';

-- 6. Теперь создаем пользователя с правильной ролью
INSERT INTO user_profiles (
    id,
    first_name,
    last_name,
    phone,
    email,
    role,
    position,
    company_id,
    company_inn,
    can_manage_requests_independently,
    password_hash,
    is_active,
    created_at,
    updated_at
) VALUES (
    'bcc40db9-f3f6-46d2-a553-a1d8afc5fae1',
    'Новый',
    'Пользователь',
    '+7 (999) 123-45-67',
    'new.user@testcompany.ru',
    'companyResponsible',
    'Ответственное лицо',
    '00000000-0000-0000-0000-000000000001',
    '0000000001',
    true,
    'ed529eae81c9b19580f0c948d5cb8c3a3b476c5a4b53e74d7118e8814cf16201',
    true,
    NOW(),
    NOW()
);

-- 7. Проверяем, что пользователь создан
SELECT 
    id, 
    first_name, 
    last_name, 
    phone, 
    email, 
    role,
    company_inn
FROM user_profiles 
WHERE id = 'bcc40db9-f3f6-46d2-a553-a1d8afc5fae1';
