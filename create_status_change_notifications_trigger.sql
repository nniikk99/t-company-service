-- ════════════════════════════════════════════════════════════════
-- ИСПРАВЛЕНИЕ: убрана ссылка на site_id (колонка не существует)
...
-- Триггерная функция (без site_id)
...
BEGIN
  ...
  IF NEW.request_number IS NOT NULL THEN
    v_short_id := LPAD(NEW.request_number::TEXT, 6, '0');
  ELSE
  ...
  IF v_recipient_ids IS NOT NULL THEN
    ...
    LOOP
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'notify_status: ошибка для %: %', v_uid, SQLERRM;
      END;
    END LOOP;
  END IF;
...
-- Проверка
SELECT tgname,
       CASE tgenabled WHEN 'O' THEN '✅ Включён' WHEN 'D' THEN '❌ Отключён' ELSE tgenabled::TEXT END AS status
FROM pg_trigger
WHERE tgname IN ('trg_notify_request_status_change','trg_notify_new_request_message');